...
public class estudiar_solicitudes extends JFrame{
	//-----------------------------------------------
	...
	private JButton               bt_consultar, bt_aprobar, bt_rechazar;
	private estudiar_solicitudes  framees = this;
	
	//-------------------------------------------
	
	public estudiar_solicitudes() {
	...
	public void setupWidgets() {
		...
		bt_aprobar = new JButton("Aprobar");
		bt_aprobar.setForeground(new Color(51, 204, 51));
		bt_aprobar.setFont(new Font("Tahoma", Font.ITALIC, 19));
		
		bt_rechazar = new JButton("Rechazar");
		bt_rechazar.setForeground(new Color(255, 0, 51));
		bt_rechazar.setFont(new Font("Tahoma", Font.ITALIC, 19));
		...
		getContentPane().add(bt_aprobar); bt_aprobar.setBounds(163, 377, 132, 47);
		
		getContentPane().add(bt_rechazar); bt_rechazar.setBounds(393, 377, 139, 47);
	...
	public void setupEvents() {
		...
		bt_aprobar.addActionListener(new ActionListener() {
			
			@Override
			public void actionPerformed(ActionEvent e) {
				JTextField ap = new JTextField("APROBADO");
				
				Connection con = null;
				
				try {
					con = getConnection();
					ps = (PreparedStatement) con.prepareStatement("UPDATE solicitudes SET estado = ? WHERE id = ?");
					ps.setString(1, ap.getText());
					ps.setString(2, tf_consultar.getText());
					
					int res = ps.executeUpdate();
					
					if(res>0) {
						add(new aprobada(framees));
					} else {
						JOptionPane.showMessageDialog(null, "Te pelaste salchichon");
					}
		...
		bt_rechazar.addActionListener(new ActionListener() {
			
			@Override
			public void actionPerformed(ActionEvent e) {
				JTextField ap = new JTextField("RECHAZADO");
